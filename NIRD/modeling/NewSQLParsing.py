...
def plat_sql(sql_block, last_sql, idx, table_dict):
    # FROM
    if type(sql_block['from']) is str:
        ...
        from_clause = "FROM " + " join ".join(
            map(lambda t_idx: table_dict['table_list'][t_idx]['table_name'], from_table_list))
...
def preprocess():
    ...
    file_path = "../data/new/%s.json" % partition
    ...
    for sample in data:
        ...
        while stack:
            ...
            sql_plat.append(plat_sql(sql_block, sql_plat[-1], idx, table_dict))
            dump(sql_block, sql_plat[-1], None, None, idx)
        """
        if len(sql_list) > 8:
            print question
            print
            print query
            print
            for sql_d, sql_p in zip(sql_list, sql_plat[1:]):
                print sql_d
                print sql_p
                print
...
def dump(sql_block, sql_plat, a, b, idx):
